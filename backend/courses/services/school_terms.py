...
def generate_session_dates(
    ...
    """Return dates matching the schedule and supplied holiday exclusions."""
    if last_date is None:
        last_date = first_date + relativedelta(months=6)

    holiday_periods = holiday_periods or []
    public_holidays = set(public_holidays or [])
    weekday_numbers = {WEEKDAYS[day.lower()] for day in weekdays}
    ...
    while current_date <= last_date:
        valid_weekday = current_date.weekday() in weekday_numbers
        excluded_by_public_holiday = current_date in public_holidays
